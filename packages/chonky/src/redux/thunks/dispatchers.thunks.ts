...
export const thunkRequestFileAction = <Action extends FileAction>(
  ...
  payload: Action['__payloadType']
    ...
    const {selectionTransform} = action;

    if (selectionTransform) dispatch(thunkApplySelectionTransform(action));

    // Apply the effect
    const {effect} = action;
    let maybeEffectPromise: MaybePromise<boolean | undefined>;
    ...
    if (effect) {
      try {
        ...
        Logger.error(`User-defined effect function for action ${action.id} threw an ` + `error: ${(error as Error).message}`);
      }
    }
