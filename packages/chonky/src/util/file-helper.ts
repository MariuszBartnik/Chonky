...
export class FileHelper {
  ...
  public static parseDate(maybeDate: Date | string | any): Nullable<Date> {
    if (typeof maybeDate === 'string' || typeof maybeDate === 'number') {
      ...
      try {
        ...
        Logger.error(
          `Could not convert provided string/number into a date: ${(error as Error).message} `,
          'Invalid value:',
          maybeDate
        );
      }
  ...
  public static isSelected(file: Nullable<FileData>, selectedFiles?: string[]): boolean {
    if (!file || !selectedFiles) return false;
